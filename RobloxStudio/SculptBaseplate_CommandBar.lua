...
-- This script adds smooth, rounded sculpting to the Baseplate with mirroring
-- Preserves existing sculpting and adds smooth variations using rounded parts
-- Paste this into Roblox Studio Command Bar

print("=== Baseplate Sculpting Script (Smooth & Rounded) ===")
...
local GRID_SIZE = 16 -- Size of each grid cell in studs (larger = smoother, fewer parts, more rounded)
...
print("    Material: " .. tostring(baseplate.Material))

-- DON'T reset - preserve existing sculpting!
print("\n💡 Preserving existing baseplate sculpting...")
print("💡 Will add smooth variations on top of existing sculpting")

-- Delete any old sculpted overlay if it exists
local existingSculpted = workspace:FindFirstChild("SculptedBaseplateOverlay")
if existingSculpted then
	existingSculpted:Destroy()
	print("  ✓ Removed old sculpted overlay")
end
...
print("\n📐 Creating smooth sculpting grid...")
print("    Grid size: " .. gridX .. " x " .. gridZ .. " cells")
print("    Cell size: " .. GRID_SIZE .. " studs (larger = smoother)")
...
-- Create smooth sculpted overlay
print("\n🗻 Creating smooth, rounded sculpted overlay...")

-- Create a model to hold all the sculpted parts
local sculptedModel = Instance.new("Model")
sculptedModel.Name = "SculptedBaseplateOverlay"
sculptedModel.Parent = workspace

-- Create sculpted parts with smooth blending
local partCount = 0
for z = 1, gridZ do
	for x = 1, gridX do
		...
		-- Get height for this cell with smooth blending from neighbors
		local centerHeight = heightMap[z] and heightMap[z][x] or 0
		...
		if x > 1 and x < gridX and z > 1 and z < gridZ then
			...
			if #neighborHeights > 0 then
				...
				-- More aggressive blending for smoother look
				height = (centerHeight * 0.5) + (avgHeight * 0.5) -- 50/50 blend for smoother transitions
			end
		end
		
		-- Calculate part position and size
		local partY = baseplateTop + height
		local partHeight = baseplateSize.Y + math.abs(height) * 2
		...
		-- Make it smoother by using larger parts with smooth edges
		-- The larger GRID_SIZE and blending already makes it smoother
		-- You can also add a SpecialMesh for even smoother appearance if needed
...
print("  ✓ Created " .. partCount .. " smooth, rounded sculpted parts")
...
-- Keep original baseplate visible underneath (it will show through gaps)
-- The overlay parts will add the height variations on top

print("\n✅ Smooth sculpting complete!")
print("💡 Height range: " .. MIN_HEIGHT .. " to " .. MAX_HEIGHT .. " studs")
print("💡 Smoothness: " .. (SMOOTHNESS * 100) .. "%")
print("💡 Mirror point: X = " .. midX .. " (center column)")
print("💡 Using larger grid cells with smooth blending for natural, rounded look")
print("💡 Original baseplate preserved - overlay adds smooth variations")
print("💡 To adjust: Change MIN_HEIGHT, MAX_HEIGHT, SMOOTHNESS, GRID_SIZE at top of script")
